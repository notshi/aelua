...
function create_robbery(H,p1,p2)
	...
	local worst =winchance(att.bros_min,def.bros_max*2) -- the worst possible case
	local best  =winchance(att.bros_max*2,def.bros_min) -- the best  possible case

	c.display_percent=math.floor((worst+best)/2) -- the user sees this number, which is the average
...
-- win 
	...
	att.win.bros  =-math.floor(att.bros      *frand(0,100,10000))	-- att loses 0%->1% of bros
